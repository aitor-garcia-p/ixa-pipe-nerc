...
public class Annotate {
	 ...
	 * {@code StatisticalNameFinder}, {@code DictionariesNameFinder} and
	 * {@code NumericNameFinder}. In particular, if --dictTag option in CLI is off,
	 * statistical models are used (this is the default). If --dictTag is activated,
	 * it has two options, "tag" and "post": tag only tags with a gazetteer and
	 * "post" post-processes the probabilistic annotation giving priority to the
	 * gazetteer. Obviously, this option depends on the --dictPath parameter being
	 * correctly specified. The --lexer numeric option annotates numeric entities
	 * (dates, percentages, and so on) via rules. Finally, the oepc option
	 * post-process the output using the one entity per class hypothesis.
	 * 
	 * @param properties
	 *            the properties
	 * @throws IOException
	 ...
	 * {@link StatisticalNameFinder} or using the {@link DictionariesNameFinder} or
	 * a combination of those with the {@link NumericNameFinder}.
	 * 
	 * @param properties
	 ...
	 */
	// @agarciap: adding several changes to use the terms (for the lemmas) together
	// with the tokens
	public final void annotateNEs(final KAFDocument kaf) throws IOException {
		...
		for (int sent=0;sent<sentences.size();sent++) {
			...
			for (Name name : names) {
				...
				List<Term> nameTerms = kaf
						.getTermsFromWFs(Arrays.asList(Arrays.copyOfRange(tokenIds, startIndex, endIndex)));
